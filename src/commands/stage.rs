...
use stamp_core::{
    ...
    util::{base64_encode, base64_decode, Public, SerdeBinary, SerText, Timestamp},
};
...
pub fn export(txid: &str, output: &str, base64: bool) -> Result<()> {
    ...
    let transaction = if transaction.has_private() {
        ...
        let (_, new_key) = util::with_new_passphrase("Your new passphrase to encrypt this transaction", |master_key, _now| { Ok(()) }, Some(now))?;
        transaction.reencrypt(&mut rng, &master_key, &new_key)
...
pub fn import(id: &str, input: &str) -> Result<()> {
    ...
    let transaction = Transaction::deserialize_binary(trans_bytes.as_slice())
        .or_else(|_| {
            Transaction::deserialize_binary(&base64_decode(trans_bytes.as_slice())?)
        })
    ...
    let transaction = if transaction.has_private() {
        ...
        transaction.reencrypt(&mut rng, &new_key, &master_key)
            .map_err(|e| anyhow!("Error re-encrypting transaction: {}", e))?
    } else {
        transaction
    };
    let txid = transaction.id().clone();
    stage_transaction(identity.id(), transaction)
